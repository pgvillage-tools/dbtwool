...
func BuildLobPlan(totalBytes int64, lobType string, buckets []SpreadBucket, emptyLobs int64) ([]LobRowPlan, error) {
	if totalBytes < 0 {
		return nil, fmt.Errorf("totalBytes must be >= 0")
	}
	if emptyLobs < 0 {
		return nil, fmt.Errorf("emptyLobs must be >= 0")
	}
	if len(buckets) == 0 {
		...
		return nil, fmt.Errorf("at least one --spread is required when totalBytes > 0")
	}
	...
	if math.Abs(sumPct-100.0) > 0.0001 {
...
// used to shuffle the indexes of the rows to insert data is spread randomly in the end.
